...
import re.weare.app.util.ErrorLocation
import re.weare.app.util.JsonFormatter
...
class MainViewModel(
    ...
    fun sortKeys(order: re.weare.app.util.JsonFormatter.SortOrder = re.weare.app.util.JsonFormatter.SortOrder.ASC, 
                 sortBy: re.weare.app.util.JsonFormatter.SortBy = re.weare.app.util.JsonFormatter.SortBy.KEY) {
        viewModelScope.launch {
    ...
    suspend fun saveJson(name: String) {
        ...
        if (jsonToSave.isNotEmpty()) {
            savedJsonRepository.saveJson(
                re.weare.app.data.database.SavedJson(
                    name = name,
                    content = jsonToSave
                )
            )
    ...
    suspend fun getRecentFiles(limit: Int = 10): List<re.weare.app.data.database.SavedJson> {
