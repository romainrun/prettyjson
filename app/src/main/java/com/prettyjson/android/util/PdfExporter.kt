...
object PdfExporter {
    ...
    /**
     * Export JSON as .txt file
     * Tries Downloads folder first, falls back to app's Documents folder
     */
    fun exportToTxtFile(context: Context, jsonString: String, fileName: String? = null): Result<File> {
        return try {
            val sanitizedFileName = fileName?.replace("\\s+".toRegex(), "_")
                ?: "json_${SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())}"
            ...
            val txtFile = File(targetDir, "$sanitizedFileName.txt")
            txtFile.writeText(jsonString, Charsets.UTF_8)
            
            Result.success(txtFile)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
