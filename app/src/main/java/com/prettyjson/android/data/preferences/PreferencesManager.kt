...
class PreferencesManager(context: Context) {
    ...
    companion object {
        ...
        private val KEY_LINE_WRAPPING = booleanPreferencesKey("line_wrapping") // Line wrapping enabled
        private val KEY_FORMAT_ON_PASTE = booleanPreferencesKey("format_on_paste") // Auto-format on paste
    }
    ...
    // Line wrapping preference (default false)
    val lineWrapping: Flow<Boolean> = dataStore.data.map { preferences ->
        preferences[KEY_LINE_WRAPPING] ?: false
    }
    
    suspend fun setLineWrapping(enabled: Boolean) {
        dataStore.edit { preferences ->
            preferences[KEY_LINE_WRAPPING] = enabled
        }
    }
    
    // Format on paste preference (default false)
    val formatOnPaste: Flow<Boolean> = dataStore.data.map { preferences ->
        preferences[KEY_FORMAT_ON_PASTE] ?: false
    }
    
    suspend fun setFormatOnPaste(enabled: Boolean) {
        dataStore.edit { preferences ->
            preferences[KEY_FORMAT_ON_PASTE] = enabled
        }
    }
}
